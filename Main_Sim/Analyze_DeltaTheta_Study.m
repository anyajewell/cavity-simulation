...
for i = 1:numel(folders)
    ...
    S = load(ws_file, 'outputs', 'sim', 'laser', 'mirror');
    sim = S.sim; outputs = S.outputs; laser = S.laser; mirror = S.mirror;
...
d = sim.L; % cavity length
r = mirror(1).D / 2; % mirror radius
theta_0 = atan(r/d); % angle subtended by the target aperture
alpha = 0.0008 * (theta_0^2 / laser.theta_D^2); % dimensionless parameter
analytic_lossfunction = (alpha*laser.theta_D^2 + DeltaTheta.^2) ./ (theta_0.^2 + DeltaTheta.^2);

% Numeric and analytic losses, as percents
sim_loss = 100*loss_RT;
analytic_lossfunction = 100*analytic_lossfunction;

figure;
loglog(DeltaTheta(DeltaTheta>0)/laser.theta_D, sim_loss(DeltaTheta>0), 'o-', 'LineWidth', 1.5, ...
    'DisplayName', 'Simulation');
yticklabels({'0.1','1','10','100'})
hold on;
loglog(DeltaTheta(DeltaTheta>0)/laser.theta_D, analytic_lossfunction(DeltaTheta>0), '--', 'LineWidth', 1.8, ...
    'DisplayName', 'Analytic model');
grid on;
xlabel('\Delta\theta / \theta_D');
ylabel('Converged loss / round trip (%)');
title('Simulation vs Proposed Analytic Loss Function');
...
save(fullfile(root_folder, 'compiled_DeltaTheta_study.mat'), ...
    'DeltaTheta', 'loss_RT', 'nRT_used', ...
    'lossHist_cell', 'GauLHS_cell', 'GauRHS_cell', ...
    'centerx_cell', 'centery_cell', 'sim', 'outputs', 'laser', 'neg_loss_RT');
